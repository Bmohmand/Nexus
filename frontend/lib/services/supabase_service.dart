...
class ManifestSchemaNotReadyException implements Exception {
  @override
  String toString() => 'ManifestSchemaNotReady';
}

/// Direct Supabase SDK access for CRUD, auth, and storage.
/// AI operations go through [ManifestApiClient] instead.
class SupabaseService {
  ...
  Future<List<ManifestItem>> fetchItems({
    AssetDomain? domain,
    int limit = 50,
    int offset = 0,
  }) async {
    try {
      ...
      return (response as List<dynamic>)
          .map((row) => ManifestItem.fromJson(row as Map<String, dynamic>))
          .toList();
    } on PostgrestException catch (e) {
      if (e.code == 'PGRST205' ||
          (e.message.contains('manifest_items') && e.message.contains('schema cache'))) {
        throw ManifestSchemaNotReadyException();
      }
      rethrow;
    }
  }

  Future<ManifestItem?> fetchItem(String id) async {
    try {
      ...
      return ManifestItem.fromJson(response);
    } on PostgrestException catch (e) {
      if (e.code == 'PGRST205' ||
          (e.message.contains('manifest_items') && e.message.contains('schema cache'))) {
        throw ManifestSchemaNotReadyException();
